...
# BASE_DIR = os.path.abspath(os.path.join(BASELINE_DIR, os.pardir))  # parent dir!
...
parser.add_argument('--normal_results_path', type=str, default='./data/Result/eval_result/', help='Log dir [default: log]')  # where I put in: /data/method_name/summary

parser.add_argument('--data_root_path', type=str, default='data/', help='Relative path to data directory')
parser.add_argument('--gt_data_folder', type=str, default='pclouds/', help='Relative path to data directory')


parser.add_argument('--pred_data_folder', type=str, default='Conrastive_700pt_weighting_ablation', help='Relative path to data directory')

parser.add_argument('--sparse_patches', type=int, default=False, help='Evaluate on a sparse subset or on the entire point cloud')
parser.add_argument('--dataset_list', type=str,
                    default=['shapes_list_simple'], nargs='+',
                    help='list of .txt files containing sets of point cloud names for evaluation')
# default=['testset_no_noise', 'testset_low_noise', 'testset_med_noise', 'testset_high_noise', 'testset_vardensity_striped', 'testset_vardensity_gradient']
FLAGS = parser.parse_args()
EXPORT = False  # export some visualizations

GT_PC_PATH = os.path.join(BASE_DIR, FLAGS.data_root_path, FLAGS.gt_data_folder)  # gt pclouds
PRED_PC_PATH = os.path.join(BASE_DIR, FLAGS.data_root_path, FLAGS.pred_data_folder)  # pred pclouds

normal_results_path = os.path.join(FLAGS.data_root_path, FLAGS.pred_data_folder)
# results_path = os.path.abspath(os.path.join(normal_results_path, os.pardir))
sparse_patches = FLAGS.sparse_patches
pred_normals_path = PRED_PC_PATH  # where I put the predicted output normals
...
print("Now predicting method:", FLAGS.pred_data_folder)
print("Now predicting dataset: ", FLAGS.dataset_list)


for dataset in dataset_list:

    normal_gt_filenames = GT_PC_PATH + dataset + '.txt'

    normal_gt_path = GT_PC_PATH
    ...
    outdir = os.path.join(normal_results_path, 'summary_pidx/')
    ...
    for i, shape in enumerate(shape_names):
        ...
        normals_results = np.loadtxt(os.path.join(normal_results_path, shape + '_pred.xyz')).astype('float32')[:, 3:]  # slice normals from predicted .xyz
